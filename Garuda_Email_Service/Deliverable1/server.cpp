// Assignment 4
// Developing a Simple email Application

// Objective
// The objective of this assignment is to develop an end‐to‐end email application which enables the users to send emails to other users 
// and retrieve the emails destined for them.
...
class POP_Server{
    public:
        ...
        int authorizeUser(string username)
        {
...
void* client_POP(void * args)
{
    ...
    string username,password,retreive,reply;
    ...
    while(1)
    {
        ...
        int res = garudaPOPServer.authorizeUser(username.substr(5));
        if(res == 1)
        {
...
int main(int argc, char * argv[])
{
    if(argc<3)
    {
        cout<<"USAGE: "<<argv[0]<<" PORT_SMTP PORT_POP"<<endl;
        cout<<"Try Again.."<<endl;
        exit(1);
    }
    int liveThreads;
