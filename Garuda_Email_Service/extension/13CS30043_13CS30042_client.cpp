// Assignment 4A
// Communicating with a standard email server

// Objective
// This assignment is an extension to Assignment 4, and the objective of this assignment is to enable the Garuda clients 
// we have written as part of Assignment 4, communicate with standard SMTP and POP servers.
...
class SMTP_Client{
	public:
        void sendMsg(){
            ...
            sendBuf = sendBuf+"\r\n";
            if(send(connfd,sendBuf.c_str(),sendBuf.size(),0)<0) cout<<"Error sending data"<<endl;
            //else cout<<"Sent: "<<sendBuf.c_str()<<endl;
        }
        ...
        string receiveMsg(){
            ...
            output.resize(bytes);
            //char out[5000];
            int bytes_received = recv(connfd, &output[0], bytes-1,0);
            if (bytes_received<0)
            {
            ...
            if(msg.substr(0,4) == "-ERR" || msg.substr(0,3) == "550" || msg.substr(0,3) == "553") cout<< "\033[1;31m"<<msg<<"\033[0m\n";
...
class POP_Client{
	public:
        void sendMsg(){
            ...
            //send(connfd,&dataLength ,sizeof(uint32_t) ,MSG_CONFIRM); // Send the data length
            sendBuf = sendBuf+"\r\n";
            if(send(connfd,sendBuf.c_str(),sendBuf.size(),MSG_CONFIRM)<0) cout<<"Error sending data"<<endl; // Send the string
        ...
        void login()
        {
        	...
        	if(loggedIn == 0){
	        	...
	        	while(1)
	        	{
	        		...
	        		ack.clear();
	        		ack = receiveMsg();
	        		if(ack.substr(0,3) == "+OK")
	        		{
	        			break;
	        		}
	        		else if(ack.substr(0,4) == "-ERR"){
	        			continue;
	        		}
        	...
        	while(1){
	        	...
	        	cout<<"2. Retreive a Mail (Please Check the LIST first)"<<endl;
...
int main(int argc, char * argv[]){
    ...
    cout<<"SMTP: "<<clientFD<<" POP: "<<clientFD_p<<endl;

    garudaSMTPClient.connect();
    garudaPOPClient.receiveReady();
